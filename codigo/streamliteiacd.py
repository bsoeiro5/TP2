...
with tabs[0]:
    ...
    *   **Avaliação Final no Teste:** Avaliar os modelos otimizados (com e sem SMOTE) no conjunto de teste.
    ...
    st.markdown("---")
    st.title("3. Conclusões sobre os Algoritmos Utilizados")
    ...
    st.subheader("3.1. Análise Comparativa dos Modelos")
    ...
    **MLP Classifier (Rede Neural)** demonstrou potencial para capturar padrões complexos nos dados, mas exigiu um ajuste cuidadoso para evitar overfitting, especialmente considerando o tamanho limitado do dataset.
    ...
    st.subheader("3.2. Fatores Determinantes para o Sucesso Académico")
    ...
    st.subheader("3.3. Limitações e Trabalho Futuro")
...
with tabs[1]:
    ...
    if st.button("Prever"):
        ...
        for name, model in models.items():
            probabilities[name] = model.predict_proba(new_student_processed)[0][1]  # Probabilidade da classe positiva (passed=1)
        
        # Calcular a média das previsões (ensemble voting)
        ...
        st.markdown("### Resultado Final (Média dos Modelos)")
